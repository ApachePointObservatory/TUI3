...
# initialize stuff
		...
		#self.housStartTime = 0 # first time recorded from houston
		#self.houstime=0.       # time reported from houstime variable 
		#self.runStartTime = 0. # time a run is startted
...
## Misc
                ...
                self.fidBunch = list()          # bundle of fiducial data
                self.lenFidBunch = 5        # number of fids to bundle
                self.lunBunch = list()          # bundle of lunar data
                self.lenLunBunch = 5        # number of luns to bundle
...
# Laser Tuning 
            ...
            ###
                ...
                self.ampdelWdg = RO.Wdg.FloatLabel(master=tunesub2sub2,helpText = 'Amplifier Delay',relief='groove')
                self.ampdelWdg.pack(side='left',anchor='w',padx=5,pady=5)
                self.ampdelWdg.set(0.0000)

                self.ampDelUpButton = RO.Wdg.Button(
			master=tunesub2sub2,
			text="    ^    ",
			command=self.FuncCall(self.doCmd,'apollo','houston laser ampdelay up'),
			...
			master=tunesub2sub2,
			text="    v    ",
			command=self.FuncCall(self.doCmd,'apollo','houston laser ampdelay down'),
			...
			master=tunesub3,
			text="    ^ 40 ",
			command=self.FuncCall(self.doCmd,'apollo','houston laser oscvolt up40'),
                        helpText = 'Increase Oscillator Voltage by 40',
                        state = 'disabled'
		)
...
# Houston status and controls
    ...
    # dskew gate slider
                ...
                gr_houston.gridWdg(self.dskewWdg,sticky = 'ew',row=9,colSpan=2)
    ...
    # predskew gate slider
                ...
                gr_houston.gridWdg(self.predskewSlider,sticky = 'ew',row = 9, col=3,colSpan=2)
    ...
    # fiducial slider (only affects graph)
                ...
                gr_houston.gridWdg(self.fidskewWdg,sticky = 'ew',row=9,col=6,colSpan=2)
    ...
    # predskew entry
			...
			master=houstonFrame,
                ...
                gr_houston.gridWdg("predskew = ", self.predskewWdg, row=8, col=0)
...
## Keyword Callback Functions
    ...
    # list of "all" data (accesible from hippodraw)
        ...
        def newFiducial(self, fiducialData, isCurrent, keyVar=None):
                ...
                newfiducialData = fiducialData

                if len(self.fidBunch) < self.lenFidBunch:
                    self.fidBunch.append(fiducialData)
                else:
                    for q in num.arange(len(self.fidBunch)):
                        ...
                        if shotNum > -1: 
                            ...
                            chanDict = fiducialData[6]  # channel:time array
                            chanTimesCor = []
                        ...
                        # set label widgets
                            ...
                            # update Fiducial return histogram
                                            ...
                                            self.fpd_all.addRow((photoD,))
                                            self.fpd.addRow((photoD,))
		...
		newlunarData = lunarData

		if len(self.lunBunch) < self.lenLunBunch:
                    self.lunBunch.append(lunarData)
                else:
                    for q in num.arange(len(self.lunBunch)):
                        lunarData = self.lunBunch[q]
                        ...
                        if shotNum > -1: 
                            ...
                            chanDict = lunarData[5]             # channel:time array
                ...
                # list of "all" data (accesible from hippodraw)
                    ...
                    self.lunBunch = []
                    self.lunBunch.append(newlunarData)
	...
	def newBlockRemaining(self, blockremainingData, isCurrent, keyVar=None):
		...
		if not isCurrent:
			return
                ...
                if self.wdgFlag == 0:
                    self.spaceAlertWdg.configure(text='Time until next blockage: %d s' % releaseremainingData[0])
...
## Various Function definitions for buffer/clear controls and entry boxes, etc.
        ...
        def setPar(self,evt,var):
                ...
                wdgDict = {'state': self.stateWdg,'polyname': self.polynameWdg,
                           ...
                           'gatewidth': self.gatewidthWdg,'runfidgw': self.runfidgwWdg,'huntstart': self.huntstartWdg,
        ...
        def offsetGuide(self,var):
                ...
                if opticsRaster:
                    ...
                    self.offx = self.altCor*self.offMag*(sY*opticsOffx+cY*opticsOffy)
                    self.offy = self.offMag*(-cY*opticsOffx+sY*opticsOffy)    # check with Tom
