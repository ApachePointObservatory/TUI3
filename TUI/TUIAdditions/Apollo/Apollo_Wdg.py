...
#                # channel offset
                ...
                #self.chanOffset = [3.5, -20.6, 13, -36.5, 24.5, -14.5, 30.1, 14.7, 2, -15.8, 48.8, -31.3, -9, -18, 0, 9.3] # Nov 11, 2018; New method of deriving channel offsets
                #self.chanOffset = [2.4, -20.1, 12.9, -37.2, 24.9, -14.4, 30.4, 15.0, 2.0, -15.4, 48.8, -31.2, -8.8, -18.2, 0, 9.9] # Feb 22, 2019; Refined new method of chanOffs derivation, and updated date regimes
                #self.chanOffset = [1.7, -20.2, 13.9, -37.4, 24.8, -14.0, 30.6, 15.6, 1.8, -15.4, 48.6, -31.3, -8.9, -18.1, 0, 8.3] # June 22 2019: Chan 16 observed to have been exhibiting strange, "walking" behavior in regards to its offset. Seems to have settled since 190301.
                self.chanOffset = [2.3, -20.8, 14.4, -49.0, 26.4, -14.4, 35.0, 16.6, 3.0, -25.7, 50.9, -31.0, -6.7, -16.2, 0, 15.1] # Aug 12 2020: was updating ACS delays, and hadn't updated chanOffs for newer HW period.
...
#            # tracking stuff
                ...
                #self.acsNominalDelayValues = [0, 52, 85, 137]   # Changed by NRC 4/11/2019, to add one more tooth on each side of LUN comb for better coverage of range return distributions. Also in combination with phasing such that combs move to larger TDC number (earlier time), for FIDs sake, and added two teeth on the lower TDC number side.
                self.acsNominalDelayValues = [0, 52, 77, 129]   # Changed by NRC 8/12/2020, to shift LUN comb earlier given the new ephemeris places the LUN returns 200-250 TDC bins earlier (~ 5 ns). (That would equate to subtracting 10 here, however, it sometimes would have extra teeth on the later side, so trying to balance that between later and earlier sides)

                # Bias extremize values locally
                self.acsDACExtremizePostPPIRVals = []
...
5=forced on;6=forced off, but can\'t tell;7=forced on, but can\'t tell')
        ...
        def newCmdText(self,cmdTextData, isCurrent, keyVar=None):
                ...
                print('newCmdText():    hIcmdText = ', self.cmdText)
        ...
        def newACSDACExtremizeStatus(self, data, isCurrent, keyVar=None):
                ...
                if data[0] == 1:   # new sweep has started
                        ...
                        # fit a parabola
                        polyOrder = 2
                        fit = np.poly1d(np.polyfit(self.acsDACExtremizeDACVals, self.acsDACExtremizePostPPIRVals, polyOrder))
                        ...
                        #fig = self.acsSweepFig
                        #
                        ax.plot(self.acsDACExtremizeDACVals, self.acsDACExtremizePostPPIRVals, 'ko')
        ...
        def newACSDACExtremize(self, data, isCurrent, keyVar=None):
                ...
                self.acsDACExtremizePostPPIRVals.append(IRPost)
        ...
        def acsDACSweepFit(self):
                ...
                #    phi = phase
                # offset = DC offset
                guess_ampl   = 0.5*(np.max(self.acsDACSweepPostPPIRVals)-np.min(self.acsDACSweepPostPPIRVals))   #np.sqrt(2)*np.std(self.acsDACSweepPostPPIRVals)
                ...
                fit_func = lambda x, p: p[0]*np.power(np.cos((2*np.pi/p[1])*(x+p[2])),2) + p[3]
                residual_func = lambda p: fit_func(self.acsDACSweepDACVals, p) - self.acsDACSweepPostPPIRVals
