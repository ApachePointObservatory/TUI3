...
def getBitmapDict():
    ...
    retDict = {}
    for mode, bitmapName in modeDict.items():
...
class GrayImageWdg(tkinter.Frame, RO.AddCallback.BaseMixin):
    ...
    **kargs):
        ...
        self.modeWdg = RadiobuttonSet(
            ...
            helpText = (
                "Default mode",
                "Drag to adjust black and white levels (or middle-click & drag)",
                "Drag to zoom (or right-click and drag)",
            ),
            helpURL =  helpURL,
            height = 18,
            width = 18,
...
#       self.cnv["cursor"] = "tcross"
    ...
    def addAnnotation(self, annType, imPos, rad, cnvOffset=(0, 0), tags=None, isImSize=True, **kargs):
        ...
        **kargs: Additional arguments. These depend on the annotation, but may include:
        ...
        """
        if numpy.any(numpy.equal(self.dataArr,None)):
            return
    ...
    def doScrollBar(self, ijInd, scrollCmd, scrollAmt=None, c=None):
        ...
        if scrollAmt == None or (self.dataArr == None).any():
            return
    ...
    def redisplay(self):
        ...
        try:
            ...
            if numpy.any(numpy.equal(self.scaledArr,None)) or self.scaledArr.shape != self.dataArr.shape:
    ...
    def removeAnnotation(self, tag):
        ...
        """
        newDict = {}
        for tags, ann in self.annDict.items():
            if tag in tags:
                ann.delete()
            else:
                newDict[tags] = ann
    ...
    def showArr(self, arr, mask=None):
        ...
        #if arr == None:
        if numpy.any(numpy.equal(arr,None)):
            return
        ...
        try:
            ...
            if (mask != None).any() and self.stretchExcludeBits:
    ...
    def _updCurrVal(self, evt=None):
        ...
        """
        if (evt==None) or numpy.any(numpy.equal(self.dataArr,None)):
